...
이 프레임워크는 프로젝트마다 **10,000개의 미래**를 시뮬레이션합니다(엔진은 Luciano Machain, 아르헨티나 UNR의 **SimulAr v2.5**와
호환). 모든 현금흐름이 **확률변수**가 되고, 포트폴리오 전체가 반복마다 다시 계산됩니다. 끝에 남는 것은 하나의 숫자가 아니라
**당신 돈의 분포 전체**입니다:
...
- **20가지 입력 분포**, 검증된 **상관행렬**(주변분포를 정확히 보존하는 Iman-Conover), **1%~99% 백분위수**,
  그리고 SimulAr 매뉴얼과 동일한 100구간 히스토그램.
