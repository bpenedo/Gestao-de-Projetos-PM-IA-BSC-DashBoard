...
```
```sql rca_proj
select * exclude (prompt_gargalo), CASE prompt_gargalo WHEN 'Conversa/Aberto' THEN '对话/开放' WHEN 'RAG/Busca' THEN 'RAG/检索' WHEN 'Transformacao/Formato' THEN '转换/格式' WHEN 'Raciocinio/Analise' THEN '推理/分析' WHEN 'Sumarizacao' THEN '摘要' WHEN 'Geracao de Codigo' THEN '代码生成' WHEN 'Extracao de Dados' THEN '数据提取' ELSE prompt_gargalo END as prompt_gargalo from bsc.rca_projeto
```
```sql rca_inter
...
![Mapa 5D do Portfólio de Projetos de IA](/5d_zh.png?v=5)
...
> **TIR** = 项目回报 · **ILL (PI)** 大于 1 = 创造价值 · 与 **SELIC** 及**美国利率**对比（上表中各项目的真实值——列 `TIR>SELIC?`/`TIR>EUA?`）。现金流经**美元化**（USD/BRL）并按美国利率贴现 → 列 **VPL US$** 与 **PB desc. US$**。_基准（SELIC、美国利率、汇率）为占位值——在 `.env` 中调整。_

**各项目 IRR vs. 机会成本（SELIC × 美国）**

<BarChart data={vpl} x=project_name y=tir title="各项目 TIR 对比 SELIC 与美国利率" yAxisTitle="TIR（每期）" sort=true>
  <ReferenceLine y=0.105 color=warning label="SELIC (BR) ~10,5%"/>
  <ReferenceLine y=0.045 color=info label="美国利率 ~4.5%"/>
</BarChart>

**投资随时间的回收**（贴现累计 — 过零点 = 贴现回收期）

<LineChart data={vpl_fluxo} x=periodo y=cum_desc series=project_name title="每期贴现累计现金流" yAxisTitle="贴现累计 (R$)" markers=true>
...
<BarChart data={planos_pagos} x=plano y=total_iof title="各计划含 IOF 的每月总成本 (R$)" swapXY=true sort=true/>
...
> 由 **TOPSIS** 以**两种归一化**排名（向量/欧氏 + min-max/线性）；
...
<img src="/admtools/zh/swot.png" alt="SWOT do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/zh/pestel.png" alt="PESTELC do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/zh/5w4h.png" alt="5W4H do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/zh/pareto.png" alt="Pareto de falhas do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/zh/gut.png" alt="Matriz GUT do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/zh/radar.png" alt="Radar competitivo do projeto eleito" style="width:100%;border-radius:8px;"/>
...
{#each kpis as p}
<a href="/zh/projetos/{p.project_name}">▶️ {p.project_name} — PSR {p.kpi_psr}</a>
{/each}
