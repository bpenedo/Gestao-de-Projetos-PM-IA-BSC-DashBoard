...
```
```sql rca_proj
select * exclude (prompt_gargalo), CASE prompt_gargalo WHEN 'Conversa/Aberto' THEN 'שיחה/פתוח' WHEN 'RAG/Busca' THEN 'RAG/חיפוש' WHEN 'Transformacao/Formato' THEN 'המרה/פורמט' WHEN 'Raciocinio/Analise' THEN 'הסקה/ניתוח' WHEN 'Sumarizacao' THEN 'תמצות' WHEN 'Geracao de Codigo' THEN 'יצירת קוד' WHEN 'Extracao de Dados' THEN 'חילוץ נתונים' ELSE prompt_gargalo END as prompt_gargalo from bsc.rca_projeto
```
```sql rca_inter
...
![Mapa 5D do Portfólio de Projetos de IA](/5d_he.png?v=5)
...
**היכן נשרף מזומן ה-AI** (Burn Rate לפי פרויקט)
...
**תמהיל כשלים גלובלי** (Pareto בדונאט)
...
> **TIR** = תשואת הפרויקט · **ILL (PI)** מעל 1 = יוצר ערך · בהשוואה ל-**SELIC** ולריבית של **ארה״ב** (ערכים אמיתיים לכל פרויקט בטבלה למעלה — עמודות `TIR>SELIC?`/`TIR>EUA?`). התזרים **דולרי** (USD/BRL) ומהוון בריבית האמריקאית → עמודות **VPL US$** ו-**PB desc. US$**. _מדדי ייחוס (SELIC, ריבית ארה״ב, שער) הם מצייני מיקום — התאם ב-`.env`._

**IRR לפי פרויקט מול עלות אלטרנטיבית (SELIC × ארה"ב)**

<BarChart data={vpl} x=project_name y=tir title="TIR לפי פרויקט מול SELIC וריבית ארה״ב" yAxisTitle="TIR (לתקופה)" sort=true>
  <ReferenceLine y=0.105 color=warning label="SELIC (BR) ~10,5%"/>
  <ReferenceLine y=0.045 color=info label="ריבית ארה״ב ~4.5%"/>
</BarChart>

**החזר ההשקעה לאורך זמן** (מצטבר מהוון — חציית אפס = החזר מהוון)

<LineChart data={vpl_fluxo} x=periodo y=cum_desc series=project_name title="תזרים מזומנים מצטבר מהוון לפי תקופה" yAxisTitle="מצטבר מהוון (R$)" markers=true>
...
<BarChart data={planos_pagos} x=plano y=total_iof title="עלות חודשית כוללת עם IOF לפי תוכנית (R$)" swapXY=true sort=true/>
...
> דירוג לפי **TOPSIS** בשתי **נרמולים** (וקטורי/אוקלידי + min-max/לינארי); ה
...
<img src="/admtools/he/swot.png" alt="SWOT do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/he/pestel.png" alt="PESTELC do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/he/5w4h.png" alt="5W4H do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/he/pareto.png" alt="Pareto de falhas do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/he/gut.png" alt="Matriz GUT do projeto eleito" style="width:100%;border-radius:8px;"/>
...
<img src="/admtools/he/radar.png" alt="Radar competitivo do projeto eleito" style="width:100%;border-radius:8px;"/>
...
{#each kpis as p}
<a href="/he/projetos/{p.project_name}">▶️ {p.project_name} — PSR {p.kpi_psr}</a>
{/each}
